---
description: Create /brownfield-iteration command for CLI-based iteration creation
order: 4
optional: true
---
...
Criar comando CLI `/brownfield-iteration` que permite criar iterações de forma interativa via Claude Code CLI, como alternativa ao botão da UI.

**Nota:** Este comando é **opcional** - a UI (comando 02) já permite criar iterações. Este comando é útil para usuários que preferem CLI.

---

# CONTEXTO

Similar ao `/expand-project`, mas focado especificamente em:
1. Criar iterações automaticamente (não apenas adicionar features manualmente)
2. Fazer backup automático de spec e database
3. Gerar arquivo de instruções para o Initializer

**Diferença:**
- `/expand-project` = Adiciona features interativamente (manual)
- `/brownfield-iteration` = Cria iteração automática (via Initializer)
...
try:
    result = create_iteration(project_dir, instructions)
...
```bash
# Projeto existente com features
/brownfield-iteration generations/test-app

# Projeto sem features
/brownfield-iteration generations/new-app
# Deve falhar com mensagem clara

# Projeto inexistente
/brownfield-iteration generations/nonexistent
# Deve falhar com mensagem clara
```
...
Ao final:
- ✅ Comando `/brownfield-iteration` disponível via CLI
- ✅ Validação de projeto brownfield
- ✅ Criação interativa de iterações
- ✅ Alternativa à UI para usuários CLI